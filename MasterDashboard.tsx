                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4">
                     ...
                     {messages.map(msg => (
                        <div key={msg.id} className={`flex flex-col ${msg.senderRole === 'MASTER' ? 'items-end' : 'items-start'}`}>
                           <div className={`max-w-[70%] p-3 rounded-xl shadow-sm text-sm ${msg.senderRole === 'MASTER'
...
export const MasterDashboard: React.FC = () => {
   ...
   const handleExtend = async (company: Company, plan: PlanType, months: number) => {
      const baseDate = company.expiresAt > Date.now() ? new Date(company.expiresAt) : new Date();
      const newDate = new Date(baseDate);
      newDate.setMonth(newDate.getMonth() + months);

      if (window.confirm(`Renovar a licença da empresa "${company.name}"?\n\nNovo Vencimento: ${newDate.toLocaleDateString()}\nPlano: ${plan}\n(Adicionando ${months} meses)`)) {
         await dbService.extendLicense(company.id, plan, months);
         fetchCompanies();
      }
   };
                                       ...
                                       <div className="inline-block relative group">
                                          ...
                                          <div className="absolute right-0 mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg hidden group-hover:block z-50">
                                             <div className="p-2 space-y-1">
                                                <button onClick={() => handleExtend(c, PlanType.MONTHLY, 1)} className="w-full text-left text-xs px-2 py-2 hover:bg-gray-100 rounded">Renovar Mensal (+1 mês)</button>
                                                <button onClick={() => handleExtend(c, PlanType.SEMIANNUAL, 6)} className="w-full text-left text-xs px-2 py-2 hover:bg-gray-100 rounded">Renovar Semestral (+6 meses)</button>
                                                <button onClick={() => handleExtend(c, PlanType.ANNUAL, 12)} className="w-full text-left text-xs px-2 py-2 hover:bg-gray-100 rounded">Renovar Anual (+1 ano)</button>
                                             </div>
                                          </div>
                                       </div>
                                    </td>
